...
def run_quiche(adata,
    ...
    mdata['quiche'].var[design.split('~')[1]] = mdata['spatial_nhood'].obs[design.split('~')[1]].values
    mdata['quiche'].var[patient_key] = mdata['spatial_nhood'].obs[patient_key].values
...
def compute_functional_expression(mdata = None,
    ...
    # adata_func = anndata.AnnData(func_df.drop(columns = ['niche', labels_key, 'niche_cell_type', fov_key, patient_key]))
    # adata_func.obs = func_df.loc[:, ['niche', labels_key, 'niche_cell_type', fov_key, patient_key]]
    adata_func = anndata.AnnData(func_df.drop(columns = ['niche', labels_key, 'niche_cell_type', fov_key]))
    adata_func.obs = func_df.loc[:, ['niche', labels_key, 'niche_cell_type', fov_key]]
