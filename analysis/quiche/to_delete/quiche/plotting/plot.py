...
def beeswarm_prev(mdata,
        ...
        cmap[mdata[feature_key].var.groupby(annot_key)['logFC'].mean() < 0] = colors[1]
        cmap[mdata[feature_key].var.groupby(annot_key)['logFC'].mean() > 0] = colors[0]
        cmap_df['cmap'] = cmap
        fig = plt.figure(figsize=figsize)
        gs = GridSpec(1, 2, width_ratios=[1, 0.3])  # 2 columns with equal width
        ...
        ax1.set_yticklabels([])
        ax1.set_yticks([])
        ax1.set_xlabel('prop. FOV prevalence', fontsize = fontsize)
        ax1.set_ylabel('')
        ax1.set_xlim([0, 0.6])
        ...
        ax0.tick_params(labelsize= bee_axis_fontsize)
        ax1.tick_params(labelsize= bar_axis_fontsize)

        ax0.set_xlabel('log2(fold change)', fontsize = fontsize)
        ax0.set_ylabel('niche neighborhoods', fontsize = fontsize)
        if xlim is not None:
        ...
        ax0.axvline(x=0, ymin=0, ymax=1, color="black", linestyle="--")
...
def plot_niche_network(G = None, figsize = (6, 6), save_directory = 'figures',filename_save = None):
    ...
    # Draw the graph
    nx.draw(G, pos, with_labels=True, node_color=colors, width=edge_weights, ax = axes)
    if filename_save is not None:
        plt.savefig(os.path.join(save_directory, filename_save + '.png'), bbox_inches = 'tight')