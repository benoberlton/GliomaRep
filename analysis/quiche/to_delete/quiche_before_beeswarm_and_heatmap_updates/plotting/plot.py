...
def da_beeswarm_updated (mdata,
        ...
        try:
            ...
            raise RuntimeError(
                f"mdata should be a MuData object with two slots: {feature_key} and 'quiche'. Run 'milopy.count_nhoods(adata)' first."
            ) from None
        ...
        mdata['quiche'].var[design_key] = mdata[feature_key].obs[design_key].values


        adata_runner = mdata[feature_key][mdata[feature_key].obs['nhood_ixs_refined'] == 1].copy()
        adata_runner.obs[niche_key] = mdata['quiche'].var.loc[:, niche_key].values
        adata_runner = adata_runner[np.isin(adata_runner.obs[niche_key], sorted_annos)]
        adata_runner.obs[niche_key] = pd.Categorical(adata_runner.obs[niche_key], categories=sorted_annos)
        perc  = quiche.pp.compute_percentile(np.abs(mdata['quiche'].var.groupby(niche_key)['logFC'].mean()[sorted_annos]), p = percentile)
        cmap_df = pd.DataFrame(mdata['quiche'].var.groupby(niche_key)['logFC'].mean(), columns = ['logFC'])
        cmap = np.full(np.shape(mdata['quiche'].var.groupby(niche_key)['logFC'].mean())[0], 'lightgrey', dtype = 'object')
        cmap[mdata['quiche'].var.groupby(niche_key)['logFC'].mean() < -1*perc] = '#377eb8'
        cmap[mdata['quiche'].var.groupby(niche_key)['logFC'].mean() >= perc] = '#e41a1c'
        cmap_df['cmap'] = cmap
        if percentile != 0:
            sorted_annos = sorted_annos[np.where((cmap_df.loc[sorted_annos]['logFC'] < -1*perc) | (cmap_df.loc[sorted_annos]['logFC'] >= perc))[0]]
        ...
        ax.axvline(x=0, ymin=0, ymax=1, color="black", linestyle="--")
        ax.legend(loc="upper right", title=f"< {int(alpha * 100)}% spatial FDR", bbox_to_anchor=(1, 1), frameon=False, prop={'size':10}, markerscale=1)
        min_name = mdata['quiche'].var.groupby(design_key)['logFC'].mean().idxmin()
        max_name = mdata['quiche'].var.groupby(design_key)['logFC'].mean().idxmax()
        plt.title(f'{min_name} vs {max_name}', fontsize = 12)
        plt.savefig(os.path.join(save_directory, filename_save+'.pdf'), bbox_inches = 'tight')
        return sorted_annos
