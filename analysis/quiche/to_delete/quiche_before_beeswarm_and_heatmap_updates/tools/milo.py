...
def run_quiche(adata,
    ...
    scores_df = pd.DataFrame(mdata['quiche'].var.groupby(annotation_key)['SpatialFDR'].median()[mdata['quiche'].var.groupby(annotation_key)['SpatialFDR'].median() < sig_threshold])
    ...
    for i in range(0, len(nn_array)):
        ...
        for cell_type in niche.split('__'):
            ...
            if len(idx_cell_type_nn) >= min_cell_count:
                ...
                func_df = pd.concat([func_df, exp], axis = 0)

    adata_func = anndata.AnnData(func_df.drop(columns = ['niche', labels_key, 'niche_cell_type', 'fov', 'Patient_ID']))
    adata_func.obs = func_df.loc[:, ['niche', labels_key, 'niche_cell_type', 'fov', 'Patient_ID']]
    return adata_func
