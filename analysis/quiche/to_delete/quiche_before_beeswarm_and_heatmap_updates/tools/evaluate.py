...
def evaluate_graphcompass(adata,
    ...
    scores_df = pd.DataFrame({'coef_label': signif_coefs.index, 'pval': model.pvalues[model.pvalues <= sig_threshold]})
    scores_df = scores_df[scores_df['coef_label'].str.contains('condition')]
    scores_df.index = scores_df.index.str.extract(r'interaction_id\[T\.([^\]]+)\]:condition')[0].values
    scores_df.drop(columns = 'coef_label', inplace = True)
...
def evaluate_kmeans(adata,
    ...
    adata_niche = compute_kmeans(adata_niche, cells_nonn, n_clusters = n_clusters, random_state=random_state, key_added = f'kmeans_cluster_{n_clusters}')    

    df = adata_niche.obs.groupby([f'kmeans_cluster_{n_clusters}', labels_key])['label'].count().unstack()
    ...
    adata_niche.obs[f'kmeans_cluster_{n_clusters}_labeled'] = adata_niche.obs[f'kmeans_cluster_{n_clusters}'].map(annotations)

    sc.pl.matrixplot(adata_niche, var_names = adata_niche.var_names, groupby = f'kmeans_cluster_{n_clusters}_labeled', colorbar_title = 'avg. frequency', cmap = 'Purples', vmin = 0, vmax = 1, save = filename_save+'.pdf')
    cluster_stats = compute_microenv_statistics(adata_niche, fov_key = fov_key, condition_key = condition_key, labels_key = f'kmeans_cluster_{n_clusters}_labeled')
    scores_df = compute_microenv_significance(cluster_stats, fov_key = fov_key, condition_key = condition_key, labels_key = f'kmeans_cluster_{n_clusters}_labeled', condition_list = condition_list)
    scores_df = scores_df[scores_df['pval'] <= sig_threshold]
...
def run_milo(adata,
    ...
    sc.pp.neighbors(mdata[feature_key], n_neighbors = n_neighbors)
    ...
    milo.annotate_nhoods(mdata, anno_col = cluster_key)
